<div class="flex items-center border-b border-gray-500 gap-x-5">
    <x-util.modal-button>
        <x-slot:icon>
            <img src="{{ $player->img['data'] }}" class="rounded-full cursor-pointer w-14 h-14">
        </x-slot:icon>
    ...
    <p class="w-5 text-xl text-gray-200">{{ $player->number ?? '-' }}</p>
    <p class="text-2xl text-gray-200">{{ $player->name }}</p>
</div>